...
public class Comment extends WeiboResponse implements java.io.Serializable {
	...
	private String createdAtStr;               //评论时间的字符串表示
	...
	/*package*/public Comment(Response res) throws WeiboException {
		...
		try {
			...
			String tmpSource = json.getString("source");
			if(tmpSource !=null && tmpSource.trim().length()>0){
				source = new Source(json.getString("source"));
			}
			createdAt = parseDate(json.getString("created_at"), "EEE MMM dd HH:mm:ss z yyyy");
			createdAtStr = json.getString("created_at");
			if(!json.isNull("user"))
				user = new User(json.getJSONObject("user"));
			if(!json.isNull("status")){
				//Log.i("comment", json.getJSONObject("status").toString());
			...
			if(!json.isNull("reply_comment")){
				...
				//Log.i("comment", json.getJSONObject("reply_comment").toString());
			}
	...
	public void setCreatedAtStr(String createdAtStr) {
		...
		try{
			this.createdAt = parseDate(this.createdAtStr, "EEE MMM dd HH:mm:ss z yyyy");
		}catch(WeiboException excep){
			Log.i("Status", excep.getMessage());
		}	
	}
