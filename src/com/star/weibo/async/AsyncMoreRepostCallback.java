...
public class AsyncMoreRepostCallback implements AsyncDataLoader.Callback {
	...
	public AsyncMoreRepostCallback(Context context, RepostStatusXListViewProxy xListViewProxy){
		...
		try {
			...
			if(OAuthInfoManager.getInstance().doBindSinaWeiboUser(mContext))
			{
				...
				if (statusSize > 0){
					...
					moreStatusList = Sina.getInstance().getWeibo().getRepostTimeline(repostXListViewProxy.getSrcStatusId(), paging);
