...
public class AsyncImageSaver {
	...
	public void saveImage(final BitmapDrawable drawable,final String name,final String cate, final String weiboType){
		executorService.submit(new Runnable() {
			
			@Override
			public void run() {
				new LocalMemory().saveDrawable(drawable, name, cate, weiboType);
			}
		});
	}
