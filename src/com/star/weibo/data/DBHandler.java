...
public class DBHandler {
	...
	public void save(Status status, Count count) {
		...
		if (status.getRetweetedStatus() == null) {
			...
			long rtId = Long.parseLong(status.getRetweetedStatus().getId());
			String rtContent = status.getRetweetedStatus().getText();
			String rtPic = status.getRetweetedStatus().getThumbnailPic();
			long rtUid = Long.parseLong(status.getRetweetedStatus().getUser().getId());
			String rtNick = status.getRetweetedStatus().getUser()
					.getScreenName();
			save(id, content, pic, create_at, source, uid, nick, portrait,
